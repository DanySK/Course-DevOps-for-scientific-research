...
% \begin{frame}[fragile]{Dependency configurations}
%     Dependencies are bound to scopes named ``configurations'', that ultimately define the classpath.
%     \begin{itemize}
%         \item \texttt{api} --- re-exposed libraries (for libraries only): projects using this library will be able to access the dependencies listed here
%         \item \texttt{implementation} --- extends \texttt{api}, adding libraries required to compile and run the project. Inheriting projects won't be able to access them.
%         \item \texttt{testImplementation} --- extends \texttt{implementation}, including libraries for compiling and running the tests
%         \item \texttt{runtimeOnly} --- libraries required at runtime, but not at compile time (e.g. Logback, if the project uses SLF4J)
%         \item \texttt{testRuntimeOnly} --- as above, for tests
%         \item \texttt{compileOnly} --- libraries required for compiling, that should not be included into runtime
%         \item \texttt{testCompileOnly} --- as above, for tests
%     \end{itemize}
% \end{frame}
